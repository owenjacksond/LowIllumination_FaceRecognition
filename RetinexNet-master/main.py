...
parser.add_argument('--save_dir', dest='save_dir', default='../result/retinexnet', help='directory for testing outputs')
parser.add_argument('--test_dir', dest='test_dir', default='../grimace/grimace', help='directory for testing inputs')
...
def lowlight_test(lowlight_enhance):
    ...
    for subdir in os.listdir(args.test_dir):
        ori_dir = args.test_dir + subdir
        if not os.path.exists(args.save_dir + ori_dir + '/'):
            os.makedirs(args.save_dir + ori_dir + '/')
        test_low_data_name = glob(os.path.join(ori_dir) + '/*.*')
        test_low_data = []
        test_high_data = []
        for idx in range(len(test_low_data_name)):
            test_low_im = load_images(test_low_data_name[idx])
            test_low_data.append(test_low_im)
        lowlight_enhance.test(test_low_data, test_high_data, test_low_data_name, save_dir=args.save_dir + ori_dir + '/', decom_flag=args.decom)
    #================================
